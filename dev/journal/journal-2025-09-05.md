...
---
## ✅ S2-T01H: Buffered Audio Segmentation

- **Goal:** Capture and segment live mic audio when voice matches the stored voiceprint and is actively speaking.
- **Implementation:**
  - Real-time buffering via `ScriptProcessorNode`
  - RMS-based VAD with debounce smoothing (250ms)
  - Voiceprint filtering with average distance threshold
  - Grace period before interrupting buffering (250ms)
  - Cooldown window after completion (500ms) to prevent re-trigger spam
  - Logs to indicate start, progress, completion, and discard
- **Issues Resolved:**
  - VAD was too sensitive → added debounce
  - Match logs spammed console → throttled
  - Buffer kept resetting or never completing → corrected state handling
  - Git push failures due to 300MB+ snapshot files → removed via `git filter-repo`
- **Test:** Segment completion confirmed multiple times during live session.
- **Commit:** `86e6973`
- **Tag:** `S2-T01H`

## ✅ S2-T01I: Convert Buffered Audio to WAV Blob

- **Goal:** Convert Float32Array segments into valid WAV Blob for Whisper.
- **Actions:**
  - Implemented `float32ToWavBlob()` with 44-byte WAV header
  - Automatically converts when segment completes
  - Logs Blob info for dev confirmation
- **Validation:**
  - Dev console logs show proper WAV Blob (audio/wav, size: XXXX bytes)
  - Verified voice segment completes only on matched + speaking input
- **Commit:** `86e6973` → `new_commit_hash`
- **Tag:** `S2-T01I`

## ✅ S2-T01J-A: Transcription provider scaffolds

- Implemented `lib/whisperRouter.ts` with pluggable backend logic.
- Supported providers:
  - `faster-whisper` (local/offline)
  - `deepgram` (cloud API)
  - `openai` (cloud API)
- `/api/whisper` dynamically selects based on `TRANSCRIBE_PROVIDER`.
- Tested type resolution, env keys, alias config, and error handling.

→ Ready for frontend integration to POST blobs and render response.
